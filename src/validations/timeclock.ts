...
export const ClockInRequestSchema = z.object({
  ...
  scheduleId: z
    ...
    .min(
      1,
      "Schedule ID is required - you must have a valid schedule to clock in"
    ),
  clockInLocation: z
    .string()
    .max(255, "Clock-in location must be less than 255 characters")
    .optional(),
});
...
export const ClockOutRequestSchema = z.object({
  ...
  clockOutLocation: z
    .string()
    .max(255, "Clock-out location must be less than 255 characters")
    .optional(),
});
...
export const TimeAdjustmentRequestSchema = z
  .object({
    timeEntryId: z.string().min(1, "Time entry ID is required"),
    clockInTime: z
      .string()
      .datetime("Clock-in time must be a valid ISO date string")
      .optional(),
    clockOutTime: z
      .string()
      .datetime("Clock-out time must be a valid ISO date string")
      .optional(),
    reason: z
      .string()
  ...
  .refine(
    ...
    {
      message:
        "At least one time field (clockInTime or clockOutTime) must be provided",
      path: ["clockInTime"],
    }
  )
  .refine(
...
>;
export type TimeAdjustmentRequestType = z.infer<
  typeof TimeAdjustmentRequestSchema
>;
export type TodayClockStatusRequestType = z.infer<
  typeof TodayClockStatusRequestSchema
>;
